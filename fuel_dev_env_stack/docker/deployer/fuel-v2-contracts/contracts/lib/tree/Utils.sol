...
// solhint-disable-next-line func-visibility
function pathLengthFromKey(uint256 key, uint256 numLeaves)
    pure
    returns (uint256 pathLength)
{
...
// solhint-disable-next-line func-visibility
function getBitAtFromMSB(bytes32 data, uint256 position)
    pure
    returns (uint256)
{
...
// solhint-disable-next-line func-visibility
function reverseSideNodes(bytes32[] memory sideNodes)
    pure
    returns (bytes32[] memory)
{
    ...
    while (left < right) {
        (sideNodes[left], sideNodes[right]) = (
            sideNodes[right],
            sideNodes[left]
        );
        left = left + 1;
...
// solhint-disable-next-line func-visibility
function countCommonPrefix(bytes32 data1, bytes32 data2)
    pure
    returns (uint256)
{
    uint256 count = 0;
