...
library SparseMerkleTree {
    ...
    function getPtrToNode(Node memory node)
        internal
        pure
        returns (bytes32 ptr)
    {
    ...
    // solhint-disable-next-line func-visibility
    function getNodeAtPtr(bytes32 ptr)
        internal
        pure
        ...
        )
    {
        ...
        for (uint256 i = 0; i < Constants.MAX_HEIGHT; i++) {
            (variables.leftNodePtr, variables.rightNodePtr) = parseNode(
                currentNode
            );
        ...
        return (
            reverseSideNodes(
                shrinkBytes32Array(sideNodes, variables.sideNodeCount)
            ),
            variables.nodePtr,
            currentNode,
    ...
    function update(
        ...
        (sideNodes, oldLeafPtr, oldLeafNode, siblingNode) = sideNodesForRoot(
            key,
            rootPtr
        );
        ...
        if (isDefaultValue(value)) {
            newRootPtr = deleteWithSideNodes(
                key,
                sideNodes,
                oldLeafNode,
                rootPtr
            ...
            newRootPtr = updateWithSideNodes(
                key,
                value,
                sideNodes,
                oldLeafPtr,
    ...
    function addBranchesAndDelete(
        ...
        newRoot = addBranchesAndUpdate(
            branches,
            root,
            key,
            abi.encodePacked(Constants.ZERO)
        );
    }
